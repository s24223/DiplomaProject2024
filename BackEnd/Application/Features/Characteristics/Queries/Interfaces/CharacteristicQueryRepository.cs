...
namespace Application.Features.Characteristics.Queries.Interfaces
{
    public class CharacteristicQueryRepository : ICharacteristicQueryRepository
    {
        ...
        public IReadOnlyDictionary<CharacteristicId, DomainCharacteristic> GetCharDictionary()
        {
            ...
            return types
                .SelectMany(x => x.Value.CharacteristicDictionary)
        ...
        {
            var characteristics = GetCharDictionary();
        ...
        {
            ids = ids.ToHashSet();
            var dictionary = GetCharDictionary();
            var list = new List<DomainCharacteristic>();
            foreach (var id in ids)
            {
                if (dictionary.TryGetValue(id, out var c))
                {
                    list.Add(c);
                }
                else
                {
                    throw new CharacteristicException(Messages.CharacteristicI_CharacteristicId_NotFound);
                }
            }
