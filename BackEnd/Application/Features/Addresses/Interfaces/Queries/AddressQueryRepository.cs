    ...
    public class AddressQueryRepository : IAddressQueryRepository
    {
            ...
            )
        {
            ...
            foreach (var item in divisionIdStreetList)
            {
                ...
                var dictionaryDivisions = await _sql.GetDivisionsHierachyUpAsync(item.DivisionId, cancellation);
        ...
        {
            // Pobierz unikalne identyfikatory
            var uniqueIds = ids.Distinct();

            // Pobierz słownik z bazy danych
